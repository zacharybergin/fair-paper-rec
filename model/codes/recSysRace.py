...
dataset = pd.read_csv('Datasets/final-asian_binary.csv', index_col= 0)
...
criterion_prediction = nn.BCELoss() # normal
# criterion_prediction = nn.BCEWithLogitsLoss() # supposed to be better
...
for lambda_fairness in lambda_values:
    ...
    selected_papers.to_csv(f'SelectedPapersList_{PROTECTED_GROUP}_{lambda_fairness}.csv')
...
df_results.to_csv(f'fairness_experiment_results_{PROTECTED_GROUP}_{randomSeed}_{lambda_fairness}.csv', index=False)
