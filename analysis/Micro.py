...
for lambda_val in lambda_values:
    for df in data_frames.values():
        if lambda_val in df['lambda_fairness'].values:
            protected_micro_by_lambda[lambda_val].append(df[df['lambda_fairness'] == lambda_val]['Protected_Micro'].values[0])
        else:
...
for lambda_val, protected_micro_values in protected_micro_by_lambda.items():
    if show_only_0_and_2_5 and lambda_val not in [0, 2.5]:
        continue
    label = "Baseline" if lambda_val == 0 else f"lambda_fairness = {lambda_val}"
    plt.plot(paper_counts, protected_micro_values, marker='o', linestyle='-', label=label)

plt.xlabel("Number of Papers")
plt.ylabel("Protected Micro (%)")
if file_path == 'fairness_outputs_cont.xlsx':
    plt.title("Protected Micro (%) vs. Number of Papers (λ Tuning, Continuous Race)")
else:
    plt.title("Protected Micro (%) vs. Number of Papers (λ Tuning, Boolean Race)")
plt.legend()
plt.grid()
plt.show()
